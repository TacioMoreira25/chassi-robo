"""
===============================================================================
ROBÔ DE INSPEÇÃO - MOTORREDUTOR JGB37-520 (12V DC)
===============================================================================
Componente: Representação CAD do motorredutor de 12V com redução de 37mm
Dimensões: Caixa de redução Ø37mm x 22mm, corpo motor Ø36mm x 30mm
Eixo motriz Ø6mm prolongado para acoplamento direto sem folgas no colar do flange
...
def criar_motor_jgb37():
    """
    Gera o motorredutor JGB37-520 integrando o modelo CAD real com eixo Ø6mm
    dimensionado para abraçamento completo pelo colar do flange de alumínio de 6mm.
    """
    caminho_step = os.path.join(os.path.dirname(__file__), "..", "modelos_cad", "motor_jgb37_520.step")
    if os.path.exists(caminho_step):
        try:
            peca_base = import_step(caminho_step).moved(Rotation(0, 180, 0))
            # Prolongamento contínuo do eixo Ø6mm para penetração no colar do flange
            with BuildPart() as extensao_eixo:
                with Locations((0, 0, 31.0)):
                    Cylinder(radius=3.0, height=22.0)
            peca = peca_base + extensao_eixo.part
    ...
    comp_eixo = 42.0                            # Eixo motriz estendido
    ...
    with BuildPart() as motor:
        ...
        with Locations((0, 0, comp_eixo / 2.0)):
